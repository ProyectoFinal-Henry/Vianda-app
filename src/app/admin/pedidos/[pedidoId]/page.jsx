...
async function DetallePedidos({ params }) {
  ...
  const fechaFormateada = new Intl.DateTimeFormat().format(
    new Date(data.fecha)
  );

  return (
    <RowResponsive>
      <h1 className="font-extrabold text-2xl md:text-3xl mb-10">
        PEDIDO #{data.id}
      </h1>
          ...
          <table className="table table-zebra">
            <thead>
              <tr className="bg-green-400">
                ...
                <th className="text-neutral">Cantidad</th>
                <th className="text-neutral">Precio Unitario</th>
                <th className="text-neutral">Total</th>
              </tr>
            </thead>
            <tbody>
              {data.detallePedido.map(
                ({ viandaNombre, precio, cantidad, total }, i) => {
                  return (
                    <tr key={i}>
                      <td>{viandaNombre}</td>
                      <td>{cantidad}</td>
