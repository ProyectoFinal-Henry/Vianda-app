...
async function ListaPedidos({ searchParams }) {
  ...
  console.log("file: page.jsx:14  querytosend:", querytosend)
    ...
    <div>
      ...
      <RowResponsive>
        <div className="flex flex-col mt-10 pb-4 items-center  w-full">
          <h1 className="font-extrabold text-2xl md:text-3xl ">Lista de Pedidos del sistema</h1>
        </div>
        <div className="overflow-x-auto max-w-full">
          <table className="table table-zebra ">
            ...
            <tbody>
              ...
              {Array.isArray(data) ? (
                data.map(({ id, usuario, estado, metodoPago, fecha, idTransaccion, totalVenta }, I) => {
                  return (
                    <tr
                      key={I}
                    ...
                    >
                      <td>{id}</td>
                      <td className="b font-bold">{usuario.nombreCompleto}</td>
                      <td className="text-center">
                        <div className="badge badge-neutral">{estado}</div>
                      </td>
                      <td className=" ">{metodoPago}</td>
                      <td className="text-center">{new Intl.DateTimeFormat().format(new Date(fecha))}</td>
                ...
                <tr>
                  <td colSpan={"7"}>
                    <h1 className=" w-full text-2xl md:text-3xl text-center">{data.message}</h1>
                  </td>
                </tr>
              )}
            </tbody>
